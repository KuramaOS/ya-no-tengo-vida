...
import { EmpleadoService, EntityArrayResponseType } from '../service/empleado.service';
...
export class EmpleadoComponent implements OnInit {
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.empleados = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IEmpleado[] | null): IEmpleado[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const empleadosNew = this.empleados ?? [];
      if (data) {
        for (const d of data) {
          if (empleadosNew.some(op => op.id === d.id)) {
            empleadosNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.empleadoService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
