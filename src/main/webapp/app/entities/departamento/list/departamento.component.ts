...
import { DepartamentoService, EntityArrayResponseType } from '../service/departamento.service';
...
export class DepartamentoComponent implements OnInit {
  ...
  protected readonly departamentoService = inject(DepartamentoService);
  ...
  trackId = (item: IDepartamento): number => this.departamentoService.getDepartamentoIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.departamentos = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IDepartamento[] | null): IDepartamento[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const departamentosNew = this.departamentos ?? [];
      if (data) {
        for (const d of data) {
          if (departamentosNew.some(op => op.id === d.id)) {
            departamentosNew.push(d);
          }
        }
      }
      return departamentosNew;
    }
    return data ?? [];
  }
